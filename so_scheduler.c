...
static struct so_scheduler *scheduler = NULL;
...
DECL_PREFIX int so_init(unsigned int time_quantum, unsigned int io)
{
	...
	scheduler->wait_queue = (struct queue **) malloc(io * sizeof(struct queue *));
	...
	scheduler->ready_queue = (struct queue **) malloc((SO_MAX_PRIO + 1) * sizeof(struct queue *));
	...
	for (i = io - 1; i >= 0; i--) {
		...
		if (scheduler->wait_queue[i] == (struct queue *)MALLOC_NULL_POINTER) {
			for (j = i + 1; j < io; j++)
	...
	for (i = SO_MAX_PRIO; i >= 0; i--) {
		...
		if (scheduler->ready_queue[i] == (struct queue *)MALLOC_NULL_POINTER) {
...
static struct so_thread *pop_next_running_thread()
{
	int i;
...
static struct so_thread *get_next_running_thread()
{
	int i;
...
static void check_scheduler(struct so_thread *myself)
{
	...
	if (crt_thread == myself) {
		...
		if (myself->time_quantum == scheduler->time_quantum || (next_thread != NULL && next_thread->priority > myself->priority)) {
			myself->time_quantum = 1;
			if (myself->status == RUNNING) {
				myself->status = READY;
				insert_into_queue(myself, scheduler->ready_queue[myself->priority]);
			}
			scheduler->current_thread = pop_next_running_thread();
		} else {
			myself->time_quantum++;
...
{
	struct so_thread *this_thread = (struct so_thread *)arg;
	pthread_mutex_lock(&scheduler->scheduler_opperation);
	hash_insert(scheduler->thread_hash, this_thread);
	insert_into_queue(this_thread, scheduler->ready_queue[this_thread->priority]);
...
DECL_PREFIX int so_signal(unsigned int io)
{
	...
	while (crt_thread != INVALID_INFO) {
		...
		insert_into_queue(crt_thread, scheduler->ready_queue[crt_thread->priority]);
		crt_thread = pop_queue(scheduler->wait_queue[io]);
		num_woken++;
	}
	pthread_mutex_unlock(&scheduler->scheduler_opperation);
